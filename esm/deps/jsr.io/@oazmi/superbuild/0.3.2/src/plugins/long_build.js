 ...
 * @module
*/
import { array_isEmpty, DEBUG, escapeLiteralStringForRegex, isNull, joinPaths, json_stringify, number_parseInt, parseFilepathInfo, pathToPosixPath, promiseOutside } from "../deps.js";
...
(function (LONGBUILD) {
    ...
    */
    LONGBUILD["RESOURCE_PATH_TYPE"] = "`${number}.res`";
...
export class LongBuildController {
    ...
    */
    ...
    cacheResolvedResult(args) {
        ...
        const path = pathToPosixPath(args.path), namespace = args.namespace ?? "file", key = namespace + ":" + path; // TODO: should I perform a `toLowerCase()` here? I'll need to check if esbuild identifies re-used resolved paths case-insensitively.
...
export const ${LONGBUILD.RESOURCE_VAR_NAME} = fakeGlobalThis.${LONGBUILD.RESOURCE_VAR_NAME}`;
        ...
        const original_resource_entities = this.resourceEntities, emitted_resource_entities = new Map();
        for (const [resource_id, output_path] of resourceImports) {
...
export class LongBuildStep {
    ...
    */
    ...
    pushImports(importer, imports) {
        const { path: _path, namespace: _namespace = "", resolveDir: _resolveDir = "" } = importer, importer_namespace = _namespace ? _namespace : "file", importer_path = (importer_namespace === "file") ? pathToPosixPath(_path) : _path, importer_key = importer_namespace + ":" + importer_path.toLowerCase(), controller = this.controller;
        for (const import_entity of imports) {
            const { importer = _path, namespace = importer_namespace, resolveDir: importer_resolveDir = _resolveDir, kind = "dynamic-import", external = false, with: with_attr = {}, ...rest_args } = import_entity;
            const resolveDir = importer_resolveDir ? importer_resolveDir
                : importer ? joinPaths(importer, "./")
                    : "./", well_defined_import_entity = {
                ...rest_args,
    ...
    */
    prepareLongBuildFileContent() {
        const controller = this.controller, resource_ext = controller.resourceExtensionName, all_resource_entities = controller.resourceEntities, all_resource_imports_this_build = this.resourceEntityIds;
        let all_imports_js_str = all_resource_imports_this_build.map((resource_id) => {
            const import_entity = all_resource_entities.get(resource_id);
            if (isNull(import_entity)) {
                throw new Error(`[LongBuildStep.prepareLongBuildFileContent]: failed to find resource with the id: "${resource_id}". this is unusual!`);
            }
        ...
        const format = controller.format, deps_filename = controller.depsFilename, next_filename = `${this.buildNumber + 1}${controller.baseFilename}`, deps_import_statement = `import { ${LONGBUILD.RESOURCE_VAR_NAME}, console_log } from "${deps_filename}"`, recursion_import_statement = !array_isEmpty(all_resource_imports_this_build)
...
export const longBuildPluginSetup = (config) => {
    const controller = config.controller, longbuild_base_filename = controller.baseFilename, longbuild_deps_filename = controller.depsFilename, longbuild_res_ext = controller.resourceExtensionName, plugin_namespace = controller.pluginNamespace;
    return (build) => {
        ...
        filter = new RegExp(escapeLiteralStringForRegex(longbuild_base_filename) + "$"), deps_file_filter = new RegExp(escapeLiteralStringForRegex(longbuild_deps_filename) + "$"), resource_file_filter = new RegExp("(?<resourceId>\\d+)" + escapeLiteralStringForRegex(longbuild_res_ext) + "$");
        ...
        build.onResolve({ filter: resource_file_filter, namespace: plugin_namespace }, (args) => {
            const resource_id = number_parseInt(resource_file_filter.exec(args.path).groups["resourceId"]), import_entity = controller.resourceEntities.get(resource_id);
            if (isNull(import_entity)) {
            ...
            const { importer, importerKey, key, namespace, path, resolveDir, kind, with: with_attr, pluginData, pluginName, external } = import_entity, immediate_result = { path, external, namespace, pluginData, pluginName };
            ...
            const resolve_options = { importer, namespace, resolveDir, kind, with: with_attr, pluginData, pluginName };
