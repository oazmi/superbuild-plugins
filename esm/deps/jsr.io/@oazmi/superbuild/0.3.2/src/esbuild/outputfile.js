    ...
    */
    toOnEmitArgs(reEmitData) {
        const imported_entities = this.imports.map((imported_entity_node) => {
            const { key, kind, external, entity } = imported_entity_node, is_external_entity = "externalPath" in entity, outputPath = is_external_entity ? entity.externalPath : entity.outputPath, initialPath = is_external_entity ? undefined : entity.initialPath, write = is_external_entity ? false : entity.write;
