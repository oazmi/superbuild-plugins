...
export class SuperPluginBuild {
    ...
    */
    getInnerEsbuildPluginBuild() { return this[INNER_PLUGIN_BUILD]; }
    ...
    */
    async performOnTransform(args) {
        const { path, namespace, loader } = args, onTransformHandlers = this.ctx.onTransformHandlers;
    ...
    onResolve(options, callback) {
        ...
        const new_callback = async (args) => {
            ...
            if (!is_resolve_call && ((result?.errors?.length ?? 0) > 0)) {
                long_build_controller.terminate();
            }
            return result;
        };
    ...
    onLoad(options, callback) {
        const resolvedResourceRegistry = this.ctx.resolvedResourceRegistry, long_build_controller = this.ctx.longBuildController;
        const transform_interceptor_callback = async (args) => {
            const { namespace, path, suffix, with: with_attr } = args, onload_result = await callback(args);
            ...
            if ((onload_result?.errors?.length ?? 0) > 0) {
            ...
            const { contents, loader = "", resolveDir = "", pluginData } = onload_result, loaded_contents = contents, on_transform_args = {
        ...
        const resource_registry_interceptor_callback = async (args) => {
            ...
            if (!isNull(result)) {
                const { path: _path, namespace: _namespace, suffix, with: with_attr } = args, { emitData, loader, transformLoader } = additional_info, path = pathToPosixPath(_path), namespace = _namespace ? _namespace : "file", key = namespace + ":" + path, contributing_emit_file = { path, namespace, suffix, loader, transformLoader, emitData };
