 ...
 * @module
*/
import { contentsToString, isNull } from "../../../deps.js";
import { HTML_NODE_TYPE } from "../deps.js";
export const scriptInlineHandlerFilter = { nodeType: HTML_NODE_TYPE.ELEMENT, nodeName: "script" };
export const scriptInlineHandlerCallback = (args, ctx) => {
    if ("src" in (args.htmlNode.attributes ?? {})) {
        return;
    }
    const { contentStore } = ctx, { htmlNode, htmlPath, htmlNamespace } = args, child_nodes = htmlNode.children, 
...
const replaceContent = async (args, ctx) => {
    ...
    const 
    // the js script's text contents are stored as a child text node.
    child_nodes = node.children, script_text_content = child_nodes.find((child_node) => (child_node.type === HTML_NODE_TYPE.TEXT));
