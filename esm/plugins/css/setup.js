...
const cssPluginSetupBase = (build, config) => {
    ...
    build.onTransform(transformFilter, async (args) => {
        const { loader, ...rest_args } = args, { contents, imports, warnings, errors } = await extractCssDeps(build, { ...rest_args, loader: loader });
    ...
    build.onEmit(emitFilter, async (args) => {
        ...
        const imported_entities = args.imports.map((imported_entity) => {
            const { key: _key, initialPath, outputPath, external: _external, ...rest_props } = imported_entity, key = _key, 
            ...
            external = true, select_initialPath = `${css_dependency_path_prefix}${key}`, replace_outputPath = initialPath ?? outputPath;
...
const extractCssDepsPluginSetupFactory = (all_imports, build) => {
    let key_counter = 0;
...
const extractCssDeps = async (build, args) => {
    const sbuild = build, // just for helping with type annotation.
    { absWorkingDir, nodePaths, alias } = build.initialOptions, { path, contents, loader, resolveDir } = args, all_imports = [], extractCssDepsPlugin = {
