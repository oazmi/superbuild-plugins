...
export const emissionsDriverPluginSetup = (config: EmissionsDriverPluginSetupConfig): EsbuildPluginSetup => {
	...
	return (build: EsbuildPluginBuild | SuperPluginBuild) => {
		const
			base_plugin_build = ((build as SuperPluginBuild).getInnerEsbuildPluginBuild?.() ?? build),
...
const incorporateLongBuildImportedEntities = async (
	...
	for (let [importer_resolved_path, well_defined_imported_entities] of import_entities) {
		...
		const imported_entity_nodes: Array<ImportedEntityNode> = well_defined_imported_entities.map((import_entity) => {
			...
			const
				{ key, outputPath, kind: original_kind, external } = import_entity,
				kind: ImportedEntityNode["kind"] = `user-import:${original_kind}`, // this is our standard `kind` label for user imports that originate from the transformation stage.
			...
			return { key, kind, external, entity } satisfies ImportedEntityNode
		})
